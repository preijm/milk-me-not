...
const HEADERS: { column: string; label: string; className: string }[] = [
  ...
  { column: "count", label: "Ratings", className: "w-24 shrink-0 justify-end" },
  { column: "avg_rating", label: "Score", className: "w-44 shrink-0 justify-end" },
];
...
export const ResultsRankedList = ({ results, sortConfig, onSort }: ResultsRankedListProps) => (
  <div>
    ...
    <ol className="flex flex-col">
      {results.map((r, i) => {
        return (
          <li key={r.product_id} className="border-b border-story-ink/8">
            ...
            >
              <span className="story-num w-10 shrink-0 text-[1.35rem] leading-none text-story-muted-2">{i + 1}</span>
              ...
              <span className="w-24 shrink-0 text-right font-sans text-[0.9375rem] font-bold text-story-muted">
                {r.count}
              </span>
              ...
              <ScoreMark
                score={r.avg_rating}
                size="md"
                className="w-44 shrink-0 flex-row items-baseline justify-end gap-2.5"
              />
