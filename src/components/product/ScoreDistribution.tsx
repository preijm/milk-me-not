    ...
    {[...bins].reverse().map(({ tier, count }) => {
      ...
      return (
        <li key={tier.key} className="flex items-center gap-3 sm:gap-4">
          <span className="story-serif w-[4.75rem] flex-shrink-0 text-[0.9375rem] font-bold sm:w-24 sm:text-base" style={{ color: tier.color }}>
            {tier.name}
          </span>
          <span className="h-8 min-w-0 flex-1 overflow-hidden rounded-full bg-story-ink/[0.06] sm:h-9">
            <span
              className="block h-full rounded-full transition-[width] duration-500"
              style={{ width: `${pct}%`, backgroundColor: tier.color }}
            />
          </span>
          <span className="story-num w-6 flex-shrink-0 text-right text-[0.9375rem] text-story-muted sm:w-8 sm:text-base">
            {count}
          </span>
        </li>
      );
